...
export class ReportsController {
  ...
  async getData(@Query('type') type: string, @Query('startDate') startDate?: string, @Query('endDate') endDate?: string, @Query('departmentId') departmentId?: string) {
    return this.reportsService.getReportData({ type, startDate, endDate, departmentId });
  }
  ...
  @Get('export/pdf')
  @Roles(UserRole.ADMINISTRATOR, UserRole.SUPERVISOR)
  @ApiOperation({ summary: 'Export PDF report' })
